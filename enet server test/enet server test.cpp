...
#
define cloth0 cloth_hair# define cloth1 cloth_shirt# define cloth2 cloth_pants# define cloth3 cloth_feet# define cloth4 cloth_face# define cloth5 cloth_hand# define cloth6 cloth_back# define cloth7 cloth_mask# define cloth8 cloth_necklace
...
vector < WorldInfo > WorldDB::getRandomWorlds() {
  ...
  for (int i = 0; i < ((worlds.size() < 10) ? worlds.size() : 10); i++) { // load first four worlds, it is excepted that they are special
...
void sendState(ENetPeer * peer) {
  ...
  for (currentPeer = server - > peers; currentPeer < & server - > peers[server - > peerCount];
    ...
    if (isHere(peer, currentPeer)) {
      ...
      BYTE * raw = packPlayerMoving( & data);
      int
      var = 0x808000; // placing and breking
      memcpy(raw + 1, &
        var, 3);
      SendPacketRaw(4, raw, 56, 0, currentPeer, ENET_PACKET_FLAG_RELIABLE);
    }
          ...
          case 2:
            {
              ...
              if (!((PlayerInfo * )(event.peer - > data)) - > isIn) {
                ...
                for (char c: ((PlayerInfo * )(event.peer - > data)) - > displayName)
                  if (c < 0x20 || c > 0x7A)((PlayerInfo * )(event.peer - > data)) - > displayName = "Bad characters in name, remove them!";
